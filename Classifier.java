...
class Classifier implements Runnable {
            ...
            status[result].post(new Runnable() {
                ...
                public void run() {
                    switch (result) {
                        case 1:
                            camera++;
                            status[result-1].setText("Camera - " + camera);
                            break;
                        case 2:
                            calendar++;
                            status[result-1].setText("ReadCalendar - " + audio);
                            break;
                        case 3:
                            sms++;
                            status[result-1].setText("ReadSms - " + sms);
                            break;
                        case 4:
                            contact++;
                            status[result-1].setText("ReadContact - " + contact);
                            break;
                        case 5:
                            location++;
                            status[result-1].setText("RequestLocation- " + location);
                            break;
                        case 6:
                            calllog++;
                            status[result-1].setText("ReadCallLog- " + calllog);
                            break;
                        case 7:
                            audio++;
                            status[result-1].setText("AudioRecording - " + audio);
                            break;
                    }
                }
            });
    ...
    public int Infer(float[] X) {
        ...
        int maxScoreIdx = -1;
        Log.d(TAG, Arrays.toString(outputTensor.getDataAsFloatArray()));
